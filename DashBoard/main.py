...
with col1:
    ...
    fig = px.bar(category_df, x="Anio", y="TotalDue", title="POR AÑO",text=['{:.0f}'.format(x) for x in category_df["Anio"]],
                 template="seaborn")
...
with col3:
    ...
    fig = px.bar(category_df, x="Month", y="TotalDue", text=['{:}'.format(x) for x in category_df["Month"]],
...
with col3:
    ...
    fig3 = px.bar(category_df3, x="Month", y="TotalDue", text=['{:}'.format(x) for x in category_df3["Month"]],
                 template="seaborn",title="POR MES")
    st.plotly_chart(fig3, use_container_width=True, height=200)
    fig3.update_traces(text=filtered_df3["Country"], textposition="outside")
with col4:
...
with col3:
    ...
    fig2 = px.bar(category_df2, x="Month", y="TotalDue", text=['{:}'.format(x) for x in category_df2["Month"]],
                 template="seaborn",title="POR MES")
    st.plotly_chart(fig2, use_container_width=True, height=200)
    fig2.update_traces(text=filtered_df2["Continent"], textposition="outside")
with col4:
...
st.subheader("VENTAS POR TITULO")
col1,col2=st.columns((2))
Genero = st.sidebar.multiselect("Selecciona el titulo", df4["Gender"].unique())
filtered_df4 = df4[df4["Gender"].isin(Genero)]

with col1:
    category_df4 = filtered_df4.groupby(by=["Anio"], as_index=False)["TotalDue"].sum()
    fig4 = px.bar(category_df4, x="Anio", y="TotalDue", text=['${:0f}'.format(x) for x in category_df4["Anio"]],
                 template="seaborn",title="POR AÑO")
    st.plotly_chart(fig4, use_container_width=True, height=200)
    fig4.update_traces(text=filtered_df4["Gender"], textposition="outside")
with col2:
    category_df4 = filtered_df4.groupby(by=["Month"], as_index=False)["TotalDue"].sum()
    fig4 = px.bar(category_df4, x="Month", y="TotalDue", text=['{:}'.format(x) for x in category_df4["Month"]],
                  template="seaborn",title="POR MES")
    st.plotly_chart(fig4, use_container_width=True, height=200)
    fig4.update_traces(text=filtered_df4["Gender"], textposition="outside")
...
with col1:
    ...
    fig5 = px.bar(category_df5, x="Anio", y="TotalDue", text=['{:}'.format(x) for x in category_df5["Anio"]],
                 template="seaborn",title="POR AÑO")
...
with col2:
    ...
    fig5 = px.bar(category_df5, x="Month", y="TotalDue", text=['{:}'.format(x) for x in category_df5["Month"]],
                  template="seaborn",title="POR MES")
    st.plotly_chart(fig5, use_container_width=True, height=200)
    fig5.update_traces(text=filtered_df5["EmailPromotion"], textposition="outside")
...
with col1:
    ...
    fig6 = px.bar(category_df6, x="Anio", y="TotalDue", text=['{:}'.format(x) for x in category_df6["Anio"]],
                 template="seaborn",title="POR AÑO")
...
with col2:
    ...
    fig6 = px.bar(category_df6, x="Month", y="TotalDue", text=['{:}'.format(x) for x in category_df6["Month"]],
                  template="seaborn",title="POR MES")
    st.plotly_chart(fig6, use_container_width=True, height=200)
